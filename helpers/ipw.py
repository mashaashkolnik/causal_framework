...
def compute_overlap_and_trim(
    ...
    """Trim to common support and quantile-trim within arms
    The procedure is:
        1. Restrict to common support on propensity:
        2. One-sided quantile trimming inside overlap:
    """
...
def make_ipw_weights(
    ...
    """Compute stabilized, Hajek-normalized IPW weights
    """
...
def prepare_ipw_dataset(
    ...
    """Prepare dataset for IPW analysis.
    Steps:
        Overlap + quantile trimming
        Add IPW weights column 'w'
        Optionally drop rows with NaNs in specified targets
    """
...
def _asmd_weighted_np(X: np.ndarray, A: np.ndarray, w: np.ndarray) -> np.ndarray:
    """Compute Hajek-weighted ASMD for continuous features
    """
    mask1 = A == 1
    mask0 = ~mask1
    w1 = w[mask1][:, None]
...
def ate_ipw_means(
    ...
    """Compute IPW Hajek means and ATE (absolute and percent)
    """
...
def bootstrap_ipw(
    ...
    """Bootstrap IPW ATEs and balance (ASMD)
    """
    ...
    # Full-data weights and balance
    w_full = make_ipw_weights(df, treat_col, e_col, stabilize=stabilize, clip=clip)
    #balance_before = assess_balance_fast(df, features_to_check, treat_col, weights=None)
        ...
        pd.DataFrame(
            {
                ...
                "ASMD": asmd_once,
                "ASMD_med_boot": np.median(asmd_mat, axis=0),
