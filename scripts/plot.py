...
def plot_test_control_distributions(
    ...
    STYLE = {
        ...
        "mean_props": dict(
            color=(0.25, 0.25, 0.25, 1.0), linestyle="--", linewidth=1.5
        ),
    }
    ...
    for i, exp in enumerate(exposures):
        ...
        if rdis.get(exp) is not None:
            ...
            ax.axhline(
                rdi_val, color=STYLE["rdi_col"], linestyle="--", linewidth=1.2, zorder=0
            )
            ax.text(
                0.98,
                rdi_val,
        ...
        ax.legend(
            handles=[
                ...
                Line2D(
                    ...
                    [],
                    linestyle="none",
                    label=f"Control mean = {m_c:.1f}"
                    if m_c is not None
                    else "Control mean = NA",
                ),
            ],
            loc="lower left",
            frameon=False,
    ...
    fig.legend(
        handles=[
            ...
            Line2D(
                [0], [0], color=STYLE["edge_col"], lw=1.5, linestyle="--", label="Mean"
            ),
        ],
        loc="upper center",
...
def summarize_runs(
    ...
    *,
    ...
    for k in sorted(control.keys()):
        ...
        a_series = (
            _get_clean_series(asmd[k], asmd_col)
            if k in asmd
        ...
        summary_data[row_label] = {
            "% confounders with ASMD<0.1": _calc_pct(
                a_series, cfg["confounders"], 0.10
            ),
            "% direct confounders with ASMD<0.05": _calc_pct(
                a_series, cfg["direct"], 0.05
            ),
            "% structural confounders with ASMD<0.05": _calc_pct(
                a_series, cfg["structural"], 0.05
            ),
            "% negative targets significant": _calc_pct(
                p_series, cfg["negative_targets"], 0.05
            ),
            "max ASMD (confounders)": conf_vals.max()
            if not conf_vals.empty
            else np.nan,
            "mean ASMD (confounders)": conf_vals.mean()
            if not conf_vals.empty
            else np.nan,
        }
    cols = [
...
def heatmap_effects_generic(
    ...
    cmap = LinearSegmentedColormap.from_list(
        "soft_bwr_r", ["#ff5e8c", "#ffffff", "#6eb9ff"]
    )
    ...
    x_display_labels = (
        [x_labels.get(c, c) for c in effect_df.columns]
        if x_labels
        else effect_df.columns
    )
...
def plot_outcome_effects_panels_significant(
    ...
    ylabels = (
        [labels_dict.get(o, o) for o in all_outcomes] if labels_dict else all_outcomes
    )
    m = len(all_outcomes)
    y_pos = np.arange(m)
    ...
    for idx, (ax, d, label) in enumerate(zip(axes_flat, prepped, df_labels)):
        ...
        if zero_line:
            ax.axvline(
                0, linestyle="--", lw=1.25, color="#8a8a8a", alpha=0.85, zorder=0
            )
        for i in range(m):
            if pd.isna(d["lo"].iat[i]):
                continue
            color = colors[i]
            ax.hlines(
                y_pos[i],
        ...
        if annotate_p:
            for i in range(m):
                if not is_sig[i] or pd.isna(d["eff"].iat[i]):
                    continue
...
def plot_outcome_effects_panels(
    ...
    *,
    ...
    for i, (df, label) in enumerate(zip(prepped_dfs, to_plot)):
        ...
        if annotate_p:
            for j in range(m_outcomes):
                if pd.isna(d["hi"].iat[j]) or pd.isna(d["p"].iat[j]):
                    continue
                ax.text(
                    d["hi"].iat[j] * factor + 0.2,
                    y_coords[j] - p_y_offset,
                    f"p={d['p'].iat[j]:.3f}",
                    ha="left",
                    va="center",
                    fontsize=p_fontsize,
        ...
        handles.append(
            plt.Line2D(
                [0], [0], color=color, lw=ci_line_width, marker="o", label=display_name
            )
        )
    ...
    ax.spines["bottom"].set_color("gray")
    ax.legend(
        handles=handles, loc="lower center", bbox_to_anchor=(0.5, 1.01), frameon=False
    )
    fig.tight_layout()
    return fig, ax
    ...
    for i in range(m):
        ...
        if show_annotations and is_sig[i]:
            ...
            ate_val = (
                eff_val
                if not is_pct_mode
                else (eff_val / 100 * c_mean.iat[i] if c_mean is not None else np.nan)
            )
            pct_val = (
                eff_val
                if is_pct_mode
                else (eff_val / c_mean.iat[i] * 100 if c_mean is not None else np.nan)
            )

            annot_text = f"BH {p_txt} • ATE={ate_val:+.2g} • Δ={pct_val:+.1f}%"
            ax.text(
                (lo + hi) / 2 if np.isfinite(lo) else eff_val,
                y_pos[i] + text_above_offset,
                annot_text,
                ha="center",
                va="bottom",
                fontsize=10,
...
def plot_combined_matching_ipw_results(
    ...
    df_m = load_and_prep(
        f"results_matching/dataframes/{feature}_results.csv", map_matching
    )
    ...
    methods = [
        (
            ...
            "#454545",
            -0.5,
        ),  # Name, Data, SigColor, NonSigColor, Offset
    ...
    ax.invert_yaxis()
    handles = [
        plt.Line2D([0], [0], color=c, lw=3, label=f"{n} ({s})")
        for n, _, sc, nsc, _ in methods
        for c, s in [(sc, "BH significant"), (nsc, "BH non-significant")]
    ]
    ax.legend(
        handles=handles,
        loc="upper center",
...
def plot_error_bars(
    ...
    labels = [
        o.split("_target_day")[0].replace("_", " ").capitalize() for o in outcomes
    ]
    ax.set_yticks(y_pos)
