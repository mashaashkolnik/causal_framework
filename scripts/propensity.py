...
def assign_treatment_values(
    ...
    if limit:
            ...
            n2 = len(df)
            #df = df[(df[exposure_baseline] >= low) & (df[exposure_baseline] <= high)]
            #print(f"Cut {n1 - n2} values from exposure and {n2 - len(df)} from baseline, len={len(df)}")

    #print('passed cutoff')
    if method == "median":
        baseline_median = df[exposure].median()
        baseline_median = max(baseline_median, 0.05)
        if exposure == 'hours_since_last_caffeine_target_day':
            print('Coffee mode')
            #df = df.dropna(subset=['hours_since_last_caffeine_target_day'])
            baseline_median = 6
        print(f"baseline_median={baseline_median}")
        ...
        #print(f'assigning function with baseline_median {baseline_median}')
        df[target] = [
