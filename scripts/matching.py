...
def balance_summary(bal: pd.DataFrame, *, asmd_thresh: float = 0.1) -> dict:
    ...
    return {
        ...
        "pct_below_thresh": float((s <= asmd_thresh).mean() * 100)
        if len(s)
        else np.nan,
    }
...
def estimate_ate_matching_pipeline_cuped(
    ...
    for col in ["p_value", "p_value_pct"]:
        ...
        if ok.sum():
            results_df.loc[ok, f"{col}_fdr_bh"] = multipletests(p[ok], method="fdr_bh")[
                1
            ]
    ...
    unique_controls = (
        matched_df.loc[matched_df["_role"] == "control"].drop_duplicates().shape[0]
    )
    meta = {
        "n_pairs": int(n_pairs),
